...
export class DocsPanelProvider implements vscode.WebviewViewProvider {
  public static readonly viewType = 'docsinator.chatView';
  ...
  private sendConfig(): void {
    ...
    this._view?.webview.postMessage({
      ...
      defaultProduct: cfg.get<string>('defaultProduct', ''),
      defaultVersion: cfg.get<string>('defaultVersion', ''),
    });
  }

  private async handleQuery(question: string, chatId?: string): Promise<void> {
    const cfg = vscode.workspace.getConfiguration('docsinator');
    const apiUrl = cfg.get<string>('apiUrl', '').replace(/\/$/, '');
    const apiKey = cfg.get<string>('apiKey', '');
    const product = cfg.get<string>('defaultProduct', '') || undefined;
    const version = cfg.get<string>('defaultVersion', '') || undefined;
    ...
    try {
      ...
      const data = await res.json() as {
        ...
        chatId?: string;
        sources?: { document: string; version?: string }[];
