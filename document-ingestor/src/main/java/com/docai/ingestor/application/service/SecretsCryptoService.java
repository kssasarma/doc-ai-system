...
/**
 * AES-256-GCM encryption for secrets stored at rest (Confluence/Notion integration OAuth tokens).
 * Ported from documentation-bot's service of the same name/shape — both services share
 * {@code SECRETS_ENCRYPTION_KEY} (base64, must decode to exactly 32 bytes), same as they already
 * share {@code JWT_SECRET}. Optional in dev (an ephemeral key is generated; encrypted values won't
 * survive a restart — acceptable for local dev, never for prod).
 */
@Slf4j
@Service
public class SecretsCryptoService {
    ...
    public SecretsCryptoService(@Value("${app.secrets.encryption-key:}") String base64Key) {
        if (base64Key == null || base64Key.isBlank()) {
            log.warn("SECRETS_ENCRYPTION_KEY is not set — generating an ephemeral in-memory key. "
                + "Any secret encrypted this run (Confluence/Notion integration tokens) becomes "
     ...
     * a wrong/rotated key must degrade to "no value available", not a 500. */
