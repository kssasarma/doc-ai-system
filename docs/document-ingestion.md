...
document-ingestor (port 8081) accepts documents from multiple sources, parses them, chunks the content, scans for PII, generates vector embeddings, and stores everything in PostgreSQL. Document binaries are stored in S3/SeaweedFS — on the platform's default bucket or a [tenant-configured bucket](multi-tenancy.md#tenant-s3-storage).
